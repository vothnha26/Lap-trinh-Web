...
public class ForgotPasswordController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (status == ResetPasswordStatus.SUCCESS) {
		    ...
		    if (status == ResetPasswordStatus.USER_NOT_FOUND) errorMessage = "Tài khoản không tồn tại!";
		    if (status == ResetPasswordStatus.EMAIL_MISMATCH) errorMessage = "Email không khớp với tài khoản!";
		    if (status == ResetPasswordStatus.PASSWORD_MISMATCH) errorMessage = "Mật khẩu xác nhận không khớp!";
		    if (status == ResetPasswordStatus.DB_ERROR) errorMessage = "Lỗi hệ thống, vui lòng thử lại!";
